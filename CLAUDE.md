...
- **ReAct agent loop** (`src/agent/loop.py`) — implemented: Groq tool-calling, session memory, schema-validation retry, templated write-confirmations. Manually tested via `testingscripts/dev_chat.py`, five bugs found and fixed (see "Key decisions and constraints" below).
...
**`check_availability` — explicit-barber branch missed service eligibility (R-13 gap, July 2026):** `_slots_for_single_barber` (used whenever a customer names a barber directly) checked `day_off` but never `eligible_services`, unlike `_slots_best_barber_per_gap` (the no-preference branch), which filtered correctly. A customer explicitly requesting `decoloracion` with `barber="rafa"` — not configured as eligible — got real free slots and a real booking. Fixed by adding the same `eligible_services` check to both branches. Regression tests confirm the empty result comes from eligibility, not from the barber's calendar happening to be busy (`tests/test_check_availability.py`). `book_appointment` needs no equivalent check — it has no barber/service-eligibility concept at all, only `color_id` (per `docs/book_appointment_spec.md`), so the fix in `check_availability` closes the actual gap.
...
**Service vocabulary and category scope are prompt-level, not `config.py`-level:** everyday Spanish phrasing ("teñirme", "decolorarme", "cortarme el pelo") is mapped to `SERVICES` categories via an instruction in `_BEHAVIORAL_INSTRUCTIONS`, not added as literal keys to `config.py` — the underlying data model is correct as-is. Separately, when asking a customer which service to book, the agent is now instructed to only offer the 6 `SERVICES` categories; `PRICE_MENU`'s finer-grained variants surface only in response to a pricing question. Both were previously unguarded, causing the model to offer `PRICE_MENU` keys (e.g. "Corte Jubilado") as if they were distinct bookable services.

**Stale `tool_error` in session history could silently block retries (July 2026):** `_BEHAVIORAL_INSTRUCTIONS` now explicitly tells the model that a `"reason": "tool_error"` result earlier in the conversation reflects that point in time only — it must always retry the tool for a new request needing calendar data, never assume an earlier failure still applies, even if the client repeats the same request (`src/agent/loop.py:172-177`). Found via manual testing: a stale `invalid_grant` OAuth error from before a token refresh stayed in a long-lived session's history (90-minute window, no summarization — see session memory design above) and caused the model to keep reporting the system as down after it was already working again. This is a prompt-level fix, not a deterministic one — unlike the date-lookup-table pattern, whether to retry after a past failure is a judgment call the model must make per-turn, not a calculation to move into code.
